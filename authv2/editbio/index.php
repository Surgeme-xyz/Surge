<?php
$myURL = '/authv2/editbio'; //URL of this script.
if(isset($_GET['token'])){
    header('location: ?');
    die();
}
...
if(isset($_GET['set'])){
    ...
    try {
        ...
        while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
          if($row['Username']==$username){
            $newbio=sanitizeInput($_POST['displayName']);
            $updateQuery = "UPDATE Users SET Bio = :newbio WHERE Username = :username";
            $updateStatement = $pdo->prepare($updateQuery);
            $updateStatement->execute(array(':newbio' => $newbio, ':username' => $row['Username']));
    ...
    <div class="container">
        ...
        <form id="changeNameForm" action='?set<?php if(isset($_GET['forward'])){echo '&forward='.$_GET['forward'];} ?>' method='POST'>
            <div class="form-group">
                <label for="displayName">New Bio:</label>
