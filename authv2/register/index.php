...
<body>
  <div class="session">
    ...
    <form action="/authv2/create/<?php if(isset($_GET['forward'])){echo'?forward='.$_GET['forward'];}?>" method='POST' class="log-in" autocomplete="off"> 
...
function extractDomain($inputStr) {
    ...
    if (!empty($matches[1])) {
      ...
      ?>
      <p>Lets get you registered! Enter your details to access Surge:</p>
      ...
      <div class="floating-label">
        ...
        <input placeholder="Display name" type="text" name="displayname" id="displayname" autocomplete="username">
        <label for="displayname">Display name:</label>
 
      </div>
      <div class="floating-label">
        <input placeholder="Password" type="password" name="pass" id="password" autocomplete="current-password">
        <label for="password">Password:</label></div>   <div class="floating-label">
        <input placeholder="Date of birth" type="date" name="dob" id="dob" autocomplete="bday">
      ...
      <span>By clicking "Sign up" you are agreeing and accepting the Surge <a href='/tos/'>Terms of service</a> and <a href='/pp'>Privacy policy</a>.</span>
      <button type="submit" onClick="return TrySubmit(this)">Sign up</button>
      <?php if(!isset($_GET['forward'])){echo '<a href="?basic"  class="discrete" >Basic page</a>';}else{
        echo '<a class="discrete" ></a>';
      } ?>
      <a href="/authv2/<?php if(isset($_GET['forward'])){echo'?forward='.$_GET['forward'];}?>" class="discrete" id='register-link'>Login</a><a href="/authv2/forgot/<?php if(isset($_GET['forward'])){echo'?forward='.$_GET['forward'];}?>" class="discrete" id='forgot-link'>Reset my password</a>
    </form>
  </div>
...
<script async defer>function isValidEmail(email) {
...
if(document.getElementById('displayname').value.length<1){
correct=false
document.getElementById('error').innerHTML='Display name is too short.'
}
if(5>document.getElementById('email').value.length){
correct=false
document.getElementById('error').innerHTML='Email too short.'
}
if(!isValidEmail(document.getElementById('email').value)){
    correct=false
document.getElementById('error').innerHTML='Email is invalid.'
}
if(13>calculateAge(document.getElementById('dob').value)){
    correct=false
document.getElementById('error').innerHTML='You are too young to use Surge.'
}
if(document.getElementById('dob').value==''){
    correct=false
document.getElementById('error').innerHTML='Enter a date of birth.'  
}if(correct){
    setTimeout(() => {
